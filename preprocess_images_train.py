...
path_base_train = 'train'
path_base_train_destination = 'preprocess_clr_train'
...
for train_file_name in train_files:
    ...
    img = EImage.read_image('{}/{}'.format(path_base_train, train_file_name),if_read_as_grayscale=False)
    # print(max(img[:,0].tolist()))
    # print(all(max(img[:,0].tolist()))<8)
    # print(all(x < 8 for x in max(img[:,0].tolist())))
    ...
    EImage.save_image('{}/{}'.format(path_base_train_destination,image_name_destination), resized)
